...
export interface UserAcademyMapping {
  ...
  academyDetails?: {
    [academyId: string]: {
      ...
      /**
       * Permissions granted on top of the role's defaults. Only meaningful
       * for `instructor` — the academy owner can opt instructors into extra
...
// ============================================
// Instructor Link Code
// ============================================
// One-shot code (8 chars, 30min TTL) generated by an admin so a professor can
// claim an `instructor` role on the academy. The code carries the snapshot of
// extra permissions the owner wants this specific instructor to have — once
// redeemed, the values are written into UserAcademyMapping.academyDetails.
export interface InstructorLinkCode {
  id: string;
  ...
  createdAt: Date;
  expiresAt: Date;
  // Permissions to grant when this code is redeemed (snapshot, not a ref —
  // so changing settings later doesn't retroactively change pending invites).
  extraPermissions: Permission[];
  // Filled when consumed.
  usedAt?: Date;
  usedBy?: string;               // UID of the professor who redeemed
  usedByName?: string;
}
