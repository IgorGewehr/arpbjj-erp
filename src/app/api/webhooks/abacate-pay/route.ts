...
async function getAcademyAdminUserId(academyId: string): Promise<string | null> {
  ...
  if (!adminsSnapshot.empty) {
    ...
    console.log(`[WEBHOOK] Found admin ${adminUserId} in users subcollection for academy ${academyId}`);
    return adminUserId;
  }
...
async function notifyAdmin(
  ...
  try {
    ...
    if (!ownerId) {
      console.error(`[WEBHOOK] notifyAdmin: No admin found for academy ${academyId}`);
      return;
    }
